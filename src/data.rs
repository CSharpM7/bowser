...
pub mod gamemode {
    ...
    lazy_static! {
        ...
        static ref MIDBUS_SLOTS: RwLock<HashSet<i32>> = RwLock::new({
    ...
    pub fn is_Midbus(color: i32) -> bool
    {
        return (*MIDBUS_SLOTS.read().unwrap()).contains(&color);
    }
    ...
    pub fn set_gamemode(){
        ...
        println!("[smashline_bowser::data] HDR: {}",hdr_enabled);
        ...
        println!("[smashline_bowser::data] Ult-S: {}",ultS_enabled);
        {
            if hdr_enabled{
...
const IDENTIFIER: &str = "smashline_bowser";
...
fn copy_dir_all(src: impl AsRef<Path>, dst: impl AsRef<Path>) -> io::Result<()> {
    ...
    for entry in std::fs::read_dir(src)? {
        ...
        if ty.is_dir() {
            ...
            println!("[smashline_bowser::data] copied {} to {}",entry.file_name().to_str().unwrap(),dst.as_ref().to_str().unwrap());
        }
    }
...
pub fn patch_files()
{
    unsafe {
        ...
        {
            println!("[smashline_bowser::data] WTF?");
            return;
        }
        ...
        for slot in 1..slots {
            ...
            println!("[smashline_bowser::data] copied motion files to {}",destFile);
        }
...
pub fn install() {
    if inital_setup() {
        ...
        println!("[smashline_bowser::data] Couldnt find mod folder");
    }
}