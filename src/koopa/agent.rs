...
unsafe fn agent_start(fighter: &mut L2CFighterCommon)
{
    ...
    if is_training_mode() {
        VarModule::set_int(fighter.battle_object, &mut koopa::instance::int::FIREBALL_COOLDOWN_FRAME,0);
    }
    ...
    VarModule::set_flag(fighter.battle_object, &mut koopa::instance::flag::IS_MIDBUS, midbus);
}
